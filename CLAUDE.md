...
- **The reset-window functions are not pure.** `hasResetHappenedToday`, `hasResetHappenedThisWeek`, `getNextDailyReset`, and `getNextWeeklyReset` each call `new Date()` internally through `getTodayTimes`, so results depend on the wall clock and cannot be tested without faking time. They do not read the database or React state.
- The formatters in the same file — `formatDailyTime`, `formatCountdown`, and `formatResetLabel` — are pure. Keep them that way, because they are the part of this file a test suite can cover without faking time. Note that `formatResetLabel` reads the local calendar fields off the reset you give it, so a test for it must pin `TZ` even though the function is pure.
